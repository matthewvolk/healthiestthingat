$(function(){const a=$("#mainSearchInputField");a.focus(),$("#searchForm").on("submit",function(b){b.preventDefault(),$.ajax({url:"/search",method:"POST",contentType:"application/json",data:JSON.stringify({restaurantQuery:a.val()}),success:function(b){renderResults(b,a.val()),a.val(""),a.blur(),window.scrollTo(0,200)}})}),$(".restaurant-link").on("click",function(a){a.preventDefault();const b=$(this)[0].childNodes[1].dataset.restaurantImage;$.ajax({url:"/search",method:"POST",contentType:"application/json",data:JSON.stringify({restaurantQuery:b}),success:function(a){renderResults(a,b),window.scrollTo(0,200)}})})});const renderResults=(a,b)=>{if($(".hero-browser-inner").empty(),a[0].menu_item_name){b&&$(".first").append(`<p style="margin-top:0; margin-bottom:40px;">Results for '${b}'</p>`);for(let b=0;25>b;b++)$(".first").append(`
        <p>
          ${renderImage(a,b)}
